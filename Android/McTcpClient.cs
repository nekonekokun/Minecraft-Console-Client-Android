    ...
    public class McTcpClient : IMinecraftComHandler
    {
        ...
        private void StartClient(string user, string uuid, string sessionID, string server_ip, ushort port, int protocolversion, ForgeInfo forgeInfo, bool singlecommand, string command)
        {
            ...
            try
            {
                ...
                ConsoleIO.WriteLine("Version is supported.\nLogging in...");
                ...
                try
                {
                    if (handler.Login())
                    {
                        ...
                        else
                        {
                            ...
                            ConsoleIO.WriteLine("Server was successfully joined.\nType '"
                ...
                catch (Exception e)
                {
                    ...
                    ConsoleIO.WriteLine("Failed to join this server.");
                    retry = true;
                }
            }
            ...
            {
                ConsoleIO.WriteLineFormatted("§8" + e.Message);
                ConsoleIO.WriteLine("Failed to connect to this IP.");
                retry = true;
            }
        ...
        /// </summary>
        public bool SendClientSettings()
        {
            Byte[] skinparts = BitConverter.GetBytes(
                (Settings.skinpart_cape_visible ? 1 : 0) |
                (Settings.skinpart_jacket_visible ? 2 : 0) |
                (Settings.skinpart_lsleeve_visible ? 4 : 0) |
                (Settings.skinpart_rsleeve_visible ? 8 : 0) |
                (Settings.skinpart_lpants_visible ? 16 : 0) |
                (Settings.skinpart_rpants_visible ? 32 : 0) |
                (Settings.skinpart_hat_visible ? 64 : 0)
                );
               

            return handler.SendClientSettings(
                Settings.Language,
                2,  //ViewDistance
                2,  //Difficulty: normal
                0,  //chatmode: enabled
