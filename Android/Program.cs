...
namespace MinecraftClient
{
    ...
    public class Program: AsyncTask
    {
        ...
        private static System.Threading.Thread offlinePrompt = null;
        private static bool useMcVersionOnce = false;

        /// <summary>
        /// The main entry point of Minecraft Console Client
        /// </summary>
        protected override Java.Lang.Object DoInBackground(params Java.Lang.Object[] objs)
        {
            string[] args = { "" };
            lock (Android.MainActivity.reset_lock)
            {
                canceled = false;
        ...
        {
            ConsoleIO.WriteLine(String.Format("§2Console Client for MC {0} to {1} - v{2} - By ORelio & Contributors", MCLowestVersion, MCHighestVersion, Version));
            ...
            string setting_path = System.Environment.GetFolderPath(System.Environment.SpecialFolder.Personal);
            ...
            else if (System.IO.File.Exists(setting_path + "/MinecraftClient.ini"))
            {
                Settings.LoadSettings(setting_path + "/MinecraftClient.ini");
            }
            else Settings.WriteDefaultSettings(setting_path + "/MinecraftClient.ini");
            ...
            if (Settings.Login == "")
            {
                ConsoleIO.Write(ConsoleIO.basicIO ? "Please type the username or email of your choice.\n" : "Login : ");
                login = ConsoleIO.ReadLine();
                if (canceled)
                {
                    ConsoleIO.Write("This task was canceled");
                    return;
                }
                ConsoleIO.WriteLine(login);
                Settings.Login = login;
            }
            //if (Settings.Password == "" && (Settings.SessionCaching == CacheType.None || !SessionCache.Contains(Settings.Login.ToLower())))
            if (Settings.Password == "" && (Settings.SessionCaching == CacheType.None || !SessionCache.Contains(login.ToLower())))
            {
                RequestPassword();
            }
            if (canceled)
        ...
        {
            ConsoleIO.Write(ConsoleIO.basicIO ? "Please type the password for " + Settings.Login + ".\n" : "Password : ");
            //Settings.Password = ConsoleIO.basicIO ? ConsoleIO.ReadLine() : ConsoleIO.ReadPassword();
            ...
            ConsoleIO.WriteLine(new string('*', Settings.Password.Length));
            if (Settings.Password == "") { Settings.Password = "-"; }
            if (!ConsoleIO.basicIO)
            {
                //Hide password length
                Console.CursorTop--; ConsoleIO.Write("Password : <******>");
                for (int i = 19; i < Console.BufferWidth; i++) { ConsoleIO.Write(' '); }
            }
        }
        ...
        private static void InitializeClient()
        {
            ...
            else
            {
                ...
                if (result != ProtocolHandler.LoginResult.Success)
                {
                    ConsoleIO.WriteLine("Connecting to Minecraft.net...");
            ...
            if (result == ProtocolHandler.LoginResult.Success)
            {
                ...
                if (Settings.DebugMessages)
                    ConsoleIO.WriteLine("Success. (session ID: " + session.ID + ')');

                //ProtocolHandler.RealmsListWorlds(Settings.Username, PlayerID, sessionID); //TODO REMOVE

                if (Settings.ServerIP == "")
                {
                    ConsoleIO.Write("Server IP : ");
                    string ip = ConsoleIO.ReadLine();
                    if (canceled)
                    {
                        ConsoleIO.Write("This task was canceled");
                        return;
                ...
                if (protocolversion == 0)
                {
                    ConsoleIO.WriteLine("Retrieving Server Info...");
            ...
            else
            {
                ...
                //retry password
                if (result == ProtocolHandler.LoginResult.WrongPassword)
                {
                    ConsoleIO.WriteLineFormatted("Wrong password.");
                    RequestPassword();
                    InitializeClient();
                    return;
                }
        ...
        public static void Restart()
        {
            new Thread(new ThreadStart(delegate
            {
                ...
                ConsoleIO.WriteLine("Restarting Minecraft Console Client...");
                InitializeClient();
            })).Start();
        }
        ...
        public static void HandleFailure(string errorMessage = null, bool versionError = false, ChatBots.AutoRelog.DisconnectReason? disconnectReason = null)
        {
            ...
            if (Settings.interactiveMode)
            {
                if (versionError)
                {
                    ConsoleIO.Write("Server version : ");
                    Settings.ServerVersion = ConsoleIO.ReadLine();
                    ConsoleIO.WriteLine(Settings.ServerVersion);
